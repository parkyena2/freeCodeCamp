...
function myReplace(str, before, after) {
  if ((/^[A-Z]/).test(before)) {
    const firstUpper = after.substring(0, 1).toUpperCase();
    after = after.replace(/^[a-z]/, firstUpper);
  } else {
    const firstLower = after.substring(0, 1).toLowerCase();
    after = after.replace(/^[A-Z]/, firstLower);
  }
